...
public class NIMManager {
    public static void NIMSet() {
        ...
        if (inMainProcess(BSApplication.getInstance())) {
            // 注册自定义消息附件解析器
            //   NIMClient.getService(MsgService.class).registerCustomAttachmentParser(new CustomAttachParser());
            // init pinyin
            PinYin.init(BSApplication.getInstance());
            PinYin.validate();

            // 初始化UIKit模块
            NimUIKit.init(BSApplication.getInstance(), infoProvider, contactProvider);
        }
    }
    ...
    private static UserInfoProvider infoProvider = new UserInfoProvider() {
        ...
        public Bitmap getTeamIcon(String teamId) {
            ...
            // 默认图
            Drawable drawable = BSApplication.getInstance().getResources().getDrawable(R.drawable.nim_avatar_group);
