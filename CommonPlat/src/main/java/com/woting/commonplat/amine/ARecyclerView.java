...
public class ARecyclerView extends RecyclerView {
    private static final String TAG = ARecyclerView.class.getSimpleName();
    ...
    public ARecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle) {
        ...
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ARecyclerView, defStyle, 0);
        ...
        try {
            ...
            refreshEnabled = a.getBoolean(R.styleable.ARecyclerView_refreshEnabled, false);
            loadMoreEnabled = a.getBoolean(R.styleable.ARecyclerView_loadMoreEnabled, false);
            refreshHeaderLayoutRes = a.getResourceId(R.styleable.ARecyclerView_refreshHeaderLayout, -1);
            loadMoreFooterLayoutRes = a.getResourceId(R.styleable.ARecyclerView_loadMoreFooterLayout, -1);
            refreshFinalMoveOffset = a.getDimensionPixelOffset(R.styleable.ARecyclerView_refreshFinalMoveOffset, -1);
    ...
    public void setRefreshHeaderView(View refreshHeaderView) {
        if (!isRefreshTrigger(refreshHeaderView)) {
            throw new IllegalArgumentException("Refresh header view must be an implement of RefreshTrigger");
        }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (triggerCondition) {
                    ...
                    if (dy > mTouchSlop && mStatus == STATUS_DEFAULT) {
